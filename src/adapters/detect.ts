 ...
 *
 * Verified env vars per platform are listed in PLATFORM_ENV_VARS below.
 * Order matters: forks or more-specific clients must appear before parents.
 */
...
export const PLATFORM_ENV_VARS = [
  ...
  // copilot-cli — fork-local adapter used by Dom's Codex/Copilot workflows.
  // Keep before vscode-copilot config-dir fallback so explicit env wins.
