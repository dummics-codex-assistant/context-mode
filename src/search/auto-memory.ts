...
export function searchAutoMemory(
  ...
  // 4. Peer collaboration logs (optional, project-local or explicitly
  // configured). peer_collab_mcp remains a separate operational bus; this only
  // makes its shadow events searchable when timeline context is requested.
  for (const candidate of collectPeerCollabCandidates(projectDir)) {
...
function collectPeerCollabCandidates(projectDir?: string): Array<{ path: string; label: string }> {
  const roots: string[] = [];
  const configuredRoot = (process.env.PEER_COLLAB_LOGS_ROOT || "").trim();
  if (configuredRoot) roots.push(configuredRoot);
  if (projectDir) roots.push(join(projectDir, ".work", "peer-collab", "logs"));
  ...
  for (const root of roots) {
    ...
    seen.add(root);
    collectJsonlFiles(root, root, candidates, 0);
  }
...
function collectJsonlFiles(
  ...
  for (const entry of entries) {
    ...
    if (stat.isDirectory()) {
      collectJsonlFiles(root, path, candidates, depth + 1);
      continue;
    }

    if (!entry.endsWith(".jsonl") || stat.size > 1_000_000) continue;
    const relative = path.slice(root.length).replace(/^[\\/]/, "").replace(/\\/g, "/");
    candidates.push({
      path,
      label: `peer-collab/${relative}`,
    });
  }
}
