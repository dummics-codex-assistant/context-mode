...
describe("searchAutoMemory adapter dispatch", () => {
  ...
  it("indexes project-local peer collaboration shadow logs as timeline memory", () => {
    const peerLogsDir = join(projectDir, ".work", "peer-collab", "logs");
    mkdirSync(peerLogsDir, { recursive: true });
    writeFileSync(
      join(peerLogsDir, "messages.jsonl"),
      JSON.stringify({
        kind: "handoff",
        body: "Remember OMEGA-PEER-HANDOFF before touching the checkout flow.",
      }) + "\n",
      "utf-8",
    );
    ...
    expect(results.length).toBeGreaterThan(0);
    expect(results[0].source).toContain("peer-collab/messages.jsonl");
    expect(results[0].content).toContain("OMEGA-PEER-HANDOFF");
  });
});
